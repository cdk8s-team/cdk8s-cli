...
export function emitHeader(code: CodeMaker, custom: boolean) {
  ...
  if (custom) {
    code.line('import { ApiObject, ApiObjectMetadata, GroupVersionKind, Lazy } from \'cdk8s\';');
  } else {
    code.line('import { ApiObject, GroupVersionKind, Lazy } from \'cdk8s\';');
  }
  code.line('import { resolveLazy } from \'cdk8s/lib/_resolve\';');
...
export function generateConstruct(typegen: TypeGenerator, def: ApiObjectDefinition) {
  ...
  typegen.emitCustomType(constructName, code => {
    ...
    function emitManifestFactory() {
      ...
      code.line('const { metadata, ...rest } = props;');
      code.open('return {');
      code.line(`...${constructName}.${GVK_STATIC},`);
      code.line('metadata: toJson_ObjectMeta(metadata),'); // TODO: check if this breaks CRDs, patch if needed
      code.line(`lazySpec: Lazy.any({ produce: () => toJson_${propsTypeName}(resolveLazy(rest)) }),`);
      code.close('};');
      code.closeBlock();
    }
  });
